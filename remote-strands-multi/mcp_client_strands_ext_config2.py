...
from strands_tools import calculator, current_time  # Import built-in tools
...
class MultiMCPAgent:
    ...
    def __init__(self, server_configs: Dict[str, Dict], bedrock_config: Dict):
        ...
        # Add built-in tools
        self.builtin_tools = [calculator, current_time]
    ...
    def discover_tools(self):
        ...
        self.all_tools.extend(self.builtin_tools)
        print(f"   ✓ Added {len(self.builtin_tools)} built-in tools")
        ...
        print(f"   • Built-in tools: {len(self.builtin_tools)}")
        print(f"   • MCP server tools: {len(self.all_tools) - len(self.builtin_tools)}")
    ...
    def create_agent(self):
        ...
        self.agent = Agent(
            ...
            system_prompt="You are a helpful AWS expert assistant with access to AWS documentation, knowledge, calculator, and time tools."
        )

        print(f"\n✓ Agent created with {len(self.all_tools)} tools from {len(self.mcp_clients)} servers + built-in tools")
...
def run_comprehensive_tests(config_path: Optional[str] = None):
    ...
    print("=" * 70)
    print("MULTI-MCP SERVER CLIENT WITH AWS BEDROCK + BUILT-IN TOOLS")
    print("Unified Agent → AWS Bedrock → Multiple Knowledge Sources + Tools")
    print("=" * 70)
    ...
    if not multi_agent.mcp_clients:
        print("\n⚠️  No MCP servers connected. Continuing with built-in tools only.")
...
Format your output in JSON.""",
        ...
        },
        {
            ...
            "question": "What is the current time, and calculate how many seconds are in 24 hours? Then tell me about AWS Lambda timeout limits.",
    ...
    print(f"   • Built-in tools: {len(multi_agent.builtin_tools)}")
